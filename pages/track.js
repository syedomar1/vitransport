      ...
      busRoutes.forEach((route, routeIndex) => {
        route.stops.forEach((stop, stopIndex) => {
          ...
          if (stopIndex === 0) { // First stop in the route, add a marker
            new mapboxgl.Marker({
              color: getRandomColor() // Set marker color
            })
              .setLngLat([stop.coordinates.coordinates[0], stop.coordinates.coordinates[1]]) // Extract coordinates
              ...
              map.addLayer({
                ...
                source: {
                  ...
                  data: {
                    ...
                    geometry: {
                      type: 'Point',
                      coordinates: [stop.coordinates.coordinates[0], stop.coordinates.coordinates[1]] // Extract coordinates
                    }
                  }
                },
                paint: {
                  'circle-color': getRandomColor(), // Set circle color
                  'circle-radius': 5 // Adjust circle radius
                }
              });

              // Set the layer as added
              addedLayers[layerId] = true;
            }
          }
        });
      });
